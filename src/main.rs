...
use image_generation::{
    EditRequest, GenerationRequest, MAX_IMAGE_COUNT, edit_images, generate_images,
};
...
enum Command {
    ...
    /// Edit an existing image with the active Codex provider.
    Edit {
        /// Edit prompt. This value is sent only to the configured image endpoint.
        #[arg(
            long,
            required_unless_present = "prompt_env",
            conflicts_with = "prompt_env"
        ...
        /// Read the edit prompt from this environment variable.
        #[arg(long, conflicts_with = "prompt")]
        prompt_env: Option<String>,
        ...
        /// Optional mask image. Transparent regions are eligible for editing.
        #[arg(long)]
        mask: Option<PathBuf>,
        ...
        /// Overall edit and download deadline in seconds.
        #[arg(long, default_value_t = DEFAULT_TIMEOUT_SECONDS, value_parser = clap::value_parser!(u64).range(1..=3600))]
        timeout_seconds: u64,

        /// Maximum input or output image size in MiB.
        #[arg(long, default_value_t = DEFAULT_MAX_IMAGE_MIB, value_parser = clap::value_parser!(u64).range(1..=1024))]
        max_image_mib: u64,
